...
pub async fn create_tree_function(payer_secret:String, rpc_url:String) ->Result<Signature, ClientError> {
    ...
    let lamports = rpc_client.get_minimum_balance_for_rent_exemption(space).unwrap();
    ...
    let non_versioned_tx = Transaction::new_signed_with_payer(&[create_account_ix, create_tree_config_ix], Some(&payer.pubkey()), &[&merkle_tree, &payer], last_blockhash);
